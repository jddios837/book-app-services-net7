...
public static class NumbersToWords
{
  ...
  public static string ToWords(this BigInteger number)
  {
    /* 
      ...
      by Richard Carr, published at 
      http://www.blackwasp.co.uk/numbertowords.aspx
    */

    /*
      Zero Rule.
      If the value is 0 then the number in words is 'zero' and no 
      other rules apply.
    */
    if (number == 0)
    {
    ...
    /*
      Three Digit Rule.
      The integer value is split into groups of three digits starting 
      from the right-hand side. Each set of three digits is then 
      processed individually as a number of hundreds, tens and units. 
      Once converted to text, the three-digit groups are recombined 
      with the addition of the relevant scale number (thousand, million, 
      billion).
    */
    ...
    /*
      Recombination Rules.
      When recombining the translated three-digit groups, each group 
      except the last is followed by a large number name and a comma, 
      unless the group is blank and therefore not included at all. 
      One exception is when the final group does not include any 
      hundreds and there is more than one non-blank group. In this 
      case, the final comma is replaced with 'and'. eg. 
    ...
    */
    ...
    string ThreeDigitGroupToWords(int threeDigits)
    {
      ...
      /* 
        ...
        the number of hundreds is added as a word. If the three-digit 
        group is exactly divisible by one hundred, the text 'hundred' 
        is appended. If not, the text "hundred and" is appended. eg. 
        'two hundred' or 'one hundred and twelve'
      */
      ...
      /* Tens Rules.
         ...
         the appropriate '-ty' word (twenty, thirty, etc.) is added to 
         the text and followed by the name of the third digit (unless 
         the third digit is a zero, which is ignored). If the tens and 
         the units are both zero, no text is added. For any other value, 
